...
Camera::Camera() : Part(PartType::camera) {
    ...
    Capture = VideoCapture("../project_video.mp4");
    DispImage = Mat::zeros(720, 1280, CV_8UC1);
    WinSlip = Mat::zeros(720, 1280, CV_8UC1);

    Array[1280] = {0};
    LeftBaseTemp = 0;
    RightBaseTemp = 0;
    LeftBase = 10000, Thresh = 720, RightBase = 10000;
    Lock = 0;

    lxPositon = -1, rxPositon = -1, lNum = 0, rNum = 0, Cur = 0.0,
    distance = 0.0, r = 0.0;

    // cout << "Connecting to camera" << endl;

    // cout << "Connected to camera =" << raspi_cam.getId() << endl;

    raspi_cam.set(cv::CAP_PROP_FRAME_WIDTH, 1280);
    raspi_cam.set(cv::CAP_PROP_FRAME_HEIGHT, 720);
...
void Camera::Run() {
    ...
    if (0 == Lock) {
        ...
        for (int col = 0; col < Frame.cols; col++) {
            line(DispImage, Point(col, 720 - Array[col]),
                 Point(col + 1, 720 - Array[col + 1]), Scalar(255), 2, LINE_8);
        }
    ...
    distance =
        (Curb.x - Frame.cols / 2) * 3.75 / (ControlPtsR[3].x - ControlPts[3].x);
